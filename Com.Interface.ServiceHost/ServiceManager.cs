...
namespace Com.Interface.ServiceHost
{
    public class ServiceManager
    {
        ...
        private readonly string tcpBaseUrl;

        private readonly string httpBaseUrl;

        public ServiceManager()
        {
            tcpBaseUrl = ConfigurationManager.AppSettings["WcfServicesTcp"];
            httpBaseUrl = ConfigurationManager.AppSettings["WcfServicesHttp"];
        }
        ...
        public void Close()
        {
            foreach (System.ServiceModel.ServiceHost serviceHost in serviceHosts.ToArray())
            {
                ...
                {
                    Logger.Error("Error closing service: {0}", e);
                }
                finally
                {
        ...
        public void OpenHost<T1, T2>(string name)
            where T1 : class
            where T2 : IService
        {
            Uri httpUrl = new Uri(String.Format("{0}/Restaurant/{1}", httpBaseUrl, name));
            Uri tcpUrl = new Uri(String.Format("{0}/Restaurant/{1}", tcpBaseUrl, name));
        ...
        public void OpenHost<T1, T2>()
            where T1 : class
            where T2 : IService
        {
            OpenHost<T1, T2>(typeof(T1).Name);
        }
    }
